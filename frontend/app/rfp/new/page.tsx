    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-black text-slate-900 dark:text-white p-6 font-sans">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 max-w-7xl mx-auto h-[cal(100vh-100px)]">
                ...
                {/* LEFT COLUMN: Input & Steps (Fixed Spacing: 4 cols) */}
                <div className="lg:col-span-4 space-y-10"> {/* Increased space-y */}
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                        <div className="relative z-10">
                            <h2 className="text-lg font-bold mb-6 flex items-center gap-3"> {/* Increased margin */}
                                <Upload className="w-5 h-5 text-blue-500" />
                                Review Document
                            </h2>
                            {!file ? (
                                <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-10 text-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer relative mb-6"> {/* Increased padding/margin */}
                                    <input 
                                        type="file" 
                                        accept=".pdf" 
                                ...
                                <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl p-4 flex items-center justify-between mb-8"> {/* Increased padding/margin */}
                    ...
                    <div className="relative pl-8 space-y-8"> {/* Increased padding left and spacing */}
                        <div className="absolute left-8 top-4 bottom-4 w-0.5 bg-slate-200 dark:bg-slate-800 -z-10"></div>
                        {WORKFLOW_STEPS.map((step, index) => (
                            <motion.div
                                key={step.id}
                                animate={{ opacity: index <= activeStep ? 1 : 0.4 }}
                                className={`relative pl-12 cursor-pointer transition-all group ${index === activeStep ? "scale-105" : "scale-100"}`} // Increased pl for label
                                onClick={() => setActiveStep(index)}
                            >
                                <div className={`absolute left-0 top-1 w-10 h-10 rounded-full flex items-center justify-center border-2 z-10 bg-slate-50 dark:bg-black transition-all duration-500 ${
                                    index === activeStep 
                                        ? `border-${step.agentColor.split('-')[1]}-500 shadow-[0_0_15px_rgba(0,0,0,0.2)] scale-110` 
                                        : index < activeStep 
                                    ...
                                    {index < activeStep ? (
                                        ...
                                        <step.icon className={`w-5 h-5 ${index === activeStep ? step.agentColor : "text-slate-400"}`} />
                                    )}
                                </div>
                                <div className="ml-4"> {/* Added Margin Left for separation */}
                                    <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${
                                        index === activeStep ? step.agentColor : "text-slate-500"
                                    }`}>
                                        {step.title}
                                    </div>
                                    ...
                                    {index === activeStep && isAnalyzing && (
                                        <div className="text-xs text-blue-500 animate-pulse mt-2 flex items-center gap-1.5 font-medium">
